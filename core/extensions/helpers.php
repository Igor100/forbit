...
class helper
{
     ...
     * @return string
     */
    public static function iniPOST($key, $default = '')
    {
     ...
     * @return string
     */
    public static function prepareInfo($info = '', $separator = '<br>')
    {
        if (is_array($info))
     ...
     * @return string
     */
    public static function activeLink($param, $return, $default = false)
    {
        $value = iniGET($param);
     ...
     * @return string
     */
    public static function activeClass($param, $return)
    {
     ...
     * @return string
     */
    public static function returnCheck($id, $return)
    {
     ...
     * @return string
     */
    public static function returnSelect($id, $return)
    {
    ...
    public static function absRedirect($url)
    {
        header('location: ' . $url);
        exit();
    }
     ...
     * @param string $text
     */
    public static function setFlashData($name, $value)
    {
     ...
     * @return string
     */
    public static function getFlashData($name, $default = '', $clear = true)
    {
     ...
     * @return string
     */
    public static function dateForm($date, $time = false, $sep = ' ', $ms = true)
    {
        global $month_string;
     ...
     * @return string|array
     */
    public static function finFormat($data)
    {
        if (is_array($data))
    ...
    public static function create403()
    {
        if(true === CONFIG_DEBUG)
            trigger_error('Not found', E_USER_ERROR);
    ...
    public static function src($src = '', $path = '')
    {
        switch ($path) {
...
///////////////////////////////////////////////////////////////////////////
     ...
     * @uses    UTF8::split
     */
    public static function random($type = NULL, $length = 9)
    {
        if ($type === NULL) {
    ...
    public static function filterParameter($value)
    {
        if (is_array($value)) {
            foreach ($value as $key => $val) {
                $value[$key] = self::filterParameter($val);
            }
     ...
     * @return mixed
     */
    public static function getParameterNoXss($name, $default = null)
    {
        if (isset($_POST[$name])) {
            $value = self::filterParameterNoXss($_POST[$name]);
        }

        if (isset($_GET[$name])) {
            $value = self::filterParameterNoXss($_GET[$name]);
        }

        if (isset($_COOKIE[$name])) {
            $value = self::filterParameterNoXss($_COOKIE[$name]);
        }
     ...
     * @return mixed
     */
    public static function filterParameterNoXss($value)
    {
        if (is_array($value)) {
            foreach ($value as $key => $val) {
                $value[$key] = self::filterParameterNoXss($val);
            }
...
# Запомнить переменную
    public static function varSave($var, $clear = false)
    {
        if ($var === false) {
            unset($_SESSION['save']);
            return NULL;
    ...
    {
        if (isset($_POST[$name])) {
            $value = self::filterParameter($_POST[$name]);
        }

        if (isset($_GET[$name])) {
            $value = self::filterParameter($_GET[$name]);
        }

        if (isset($_COOKIE[$name])) {
            $value = self::filterParameter($_COOKIE[$name]);
        }
    ...
    public static function limitZero($data, $mod = false)
    {
        return ($data > 0) ? $data : (($mod) ? 1 : 0);
    }

    public static function filterUnsafeAttributes($data, $safe_attributes)
    {
     ...
     * @return array
     */
    public static function arrayMap($array, $from, $to, $group = null)
    {
        $result = array();
     ...
     * @return string
     */
    public static function translateWord($string)
    {
